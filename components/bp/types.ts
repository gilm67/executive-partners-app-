...
export function getROA(i: CandidateInputs, year: 1 | 2 | 3): number {
  const pct =
    year === 1 ? i.roa_y1_pct :
    year === 2 ? i.roa_y2_pct :
    i.roa_y3_pct;
  ...
  const n = Number(pct ?? legacy ?? 0);
  return Number.isFinite(n) ? n : 0;
}

// Prospect accessors that work for both shapes
export function getProspectBestNNM(p: Prospect): number {
  const n = Number(p.best_nnm_m ?? p['Best NNM (M)'] ?? 0);
  return Number.isFinite(n) ? n : 0;
}

export function getProspectWealth(p: Prospect): number {
  const n = Number(p.wealth_m ?? p['Wealth (M)'] ?? 0);
  return Number.isFinite(n) ? n : 0;
}