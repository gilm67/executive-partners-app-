...
import { linkedInArticles } from "@/lib/insights/linkedinArticles";
...
export default function LinkedInInsights() {
  const items = [...linkedInArticles].sort((a, b) => (a.dateISO < b.dateISO ? 1 : -1));
    ...
    <section className="w-full">
      <div className="mx-auto max-w-6xl px-4 py-10">
        <div className="flex items-end justify-between gap-6">
          <div>
            <p className="text-sm tracking-widest uppercase text-white/60">Insights</p>
            <h2 className="mt-2 text-2xl md:text-3xl font-semibold text-white">
              Private Wealth Pulse — Latest LinkedIn Articles
            </h2>
            <p className="mt-2 max-w-2xl text-white/70">
              Curated market intelligence on Swiss private banking, global wealth management, and recruitment signals.
            </p>
          </div>
          ...
          {items.map((a) => (
            ...
            >
              <div className="flex items-center justify-between gap-4">
                <time className="text-sm text-white/60">{formatDate(a.dateISO)}</time>
              ...
              <p className="mt-3 text-sm text-white/70 leading-relaxed">{a.summary}</p>
