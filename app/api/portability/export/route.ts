// app/api/portability/export/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    page.drawText(today(), { x: width - M - 100, y: height - 52, size: 10, font, color: ink.light });

    const cardX = M, cardW = width - M*2, cardH = 120;
    y = height - 90;
    ...
    const bc = (body.bookingCentres?.length ? body.bookingCentres.join(", ") : "—");
    page.drawText(`Market: ${mk}`, { x: cardX + 16, y: y - 46, size: 11, font, color: ink.text });
    page.drawText(`Booking centres: ${bc}`, { x: cardX + 16, y: y - 64, size: 11, font, color: ink.text });
    ...
    page.drawText("Median", { x: mdX - 16, y: barY + 16, size: 8, font, color: ink.light });
    page.drawText("Top quartile", { x: tqX - 20, y: barY + 16, size: 8, font, color: ink.light });
    page.drawText(`${score}/100`, { x: barX + barW + 8, y: barY - 1, size: 11, font: fontBold, color: ink.text });
    ...
    // Node-friendly response body
    const bytes = await pdf.save();                  // Uint8Array
    const buf = Buffer.from(bytes);                  // Node Buffer
