...
// Your LinkedIn newsletter subscribe URL
const newsletterSubscribeUrl =
  "https://www.linkedin.com/newsletters/7319810409748746241/?displayConfirmation=true";

/* ----------------- Content ----------------- */

// Newsletter issues (Private Wealth Pulse)
...
const articles: Item[] = [
  ...
  },
  {
    ...
    date: "Apr 13, 2025",
    href: "https://www.linkedin.com/pulse/navigating-trumps-economic-storm-how-private-banks-can-gil-m-chalem--9q2de/?trackingId=dJZo6qjzRUOFPP3nQ%3D%3D",
    tag: "Article",
  },
  {
    title:
...
export default function InsightsPage() {
  const [filter, setFilter] = useState<"all" | "pulse" | "articles">("all");

  const items = useMemo(() => {
    const all = [...newsletter, ...articles];
    if (filter === "pulse") return newsletter;
  ...
  return (
    <main className="relative min-h-screen text-white">
      ...
      <div className="relative mx-auto w-full max-w-6xl px-4 pb-24 pt-10">
        {/* Hero */}
        <header className="mb-8 text-center">
          ...
          <h1 className="mt-3 text-4xl font-extrabold tracking-tight">Insights</h1>
          <p className="mx-auto mt-3 max-w-3xl text-neutral-300">
            A curated stream of our <em>Private Wealth Pulse</em> newsletter and
            long-form articles for private bankers and wealth leaders.
          </p>
        </header>

        {/* Subscribe strip */}
        <section className="mb-8 overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-r from-blue-600/30 via-blue-500/20 to-emerald-500/20 p-5 shadow-[0_1px_3px_rgba(0,0,0,.25)]">
          <div className="flex flex-col items-start gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-lg font-semibold">Subscribe to Private Wealth Pulse</h2>
              <p className="mt-1 text-sm text-white/80">
                Weekly highlights on Swiss & global private banking — delivered via LinkedIn.
              </p>
            </div>
            <a
              href={newsletterSubscribeUrl}
              target="_blank"
        ...
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          ...
          <p className="text-sm text-white/70">{items.length} post{items.length === 1 ? "" : "s"}</p>
        </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((item) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-white/[0.06] to-white/[0.03] p-5 shadow-[0_1px_3px_rgba(0,0,0,.25)] transition hover:shadow-[0_12px_50px_rgba(0,0,0,.50)]"
              >
                {/* Glow on hover */}
                ...
                <div className="relative">
                  <div className="flex items-center justify-between gap-3">
                    {item.tag && (
                      <span className="inline-flex items-center rounded-full border border-white/10 bg-white/10 px-2 py-0.5 text-[11px] font-semibold text-white/90">
                        {item.tag}
                      </span>
                    )}
                    <span className="text-xs text-white/70">{item.date}</span>
                  </div>
                  <h3 className="mt-2 min-h-[3.25rem] line-clamp-3 text-lg font-semibold leading-snug">
                    {item.title}
                  </h3>
                </div>

                <div className="mt-auto pt-4">
                  <span className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/5 px-3 py-2 text-sm font-semibold text-white transition group-hover:bg-white/10">
